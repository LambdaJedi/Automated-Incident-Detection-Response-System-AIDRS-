...
public class DefaultMyListParameterized<T>
    ...
    public void add(T element) {
        ...
        final Node<T> newNode = new Node<T>(lastNode, element, null);
        last = newNode;
        if (lastNode == null) {
            first = newNode;
        } else {
    ...
    private class ListIteratorImpl extends IteratorImpl
        ...
        public T previous() {
            cursor -= 1;
            Node<T> previousNode = getNodeByIndex(cursor);
        ...
        public void set(T element) {
            ...
            Node<T> x = getNodeByIndex(lastRet);
            x.data = element;
            lastRet = -1;
        }
    ...
    private class IteratorImpl implements Iterator<T> {
        ...
        @Override
        public T next() {
            Node<T> node = getNodeByIndex(cursor);
            if (node == null) {
                throw new NoSuchElementException();
            }
            lastRet = cursor;
